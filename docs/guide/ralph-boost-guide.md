...
> **零依赖** — 不依赖 ralph-claude-code、block-break 或任何外部服务。主路径（Agent 循环）零外部依赖；Fallback 路径需要 `jq` 或 `python3` 和 `claude` CLI。
...
**Fallback（bash 脚本）**：`boost-loop.sh` 在后台运行 `claude -p` 调用循环。支持 jq 和 python3 双 JSON 引擎，运行时自动检测。每轮之间默认 sleep 1 小时（可配置）。
...
安装 jskills plugin 后，Fallback 脚本在 `~/.claude/plugins/juserch_jskills/skills/ralph-boost/scripts/boost-loop.sh`。也可以把它复制到任意位置运行。脚本自动检测 jq 或 python3 作为 JSON 引擎。
...
当前支持 Claude Code 平台（Agent 循环主路径）。Fallback bash 脚本需要 bash 4+、jq 或 python3、claude CLI。
