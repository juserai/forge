...
- **级别**: error（本规则随 `add-output-language-flag` change 一次性把全部 skill 改齐，无遗留问题，故直接刚性）
- **配置**: `.skill-lint.json` 中 `verify-lang-flag-required` 三态 (`off` / `warn` / `error`)
- **不通过**:
  - argument-hint 字段缺失 → `S35: <path> missing argument-hint with --lang`
  - argument-hint 存在但不含 `--lang` → `S35: <path> argument-hint missing --lang`
- **契约**: [openspec/specs/output-language/spec.md](../../../../openspec/specs/output-language/spec.md)
- **与 S34 协作**: S35 保证 `--lang` 进入 argument-hint；S34 保证 argument-hint 的 flag（含 `--lang`）出现在 help card——两者合起来锁住"声明 + 可见"
