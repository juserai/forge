...
控制 5 段报告正文 + 控制台输出语言。取值：`zh-CN`(默认) | en | ja | ko | fr | de | es | pt-BR | ru | tr | vi | auto。不带 `--lang` → zh-CN（覆盖任何自动检测）；`--lang <code>` → 强制该语言（inline 术语 / 论文标题可保留原文）；`--lang auto` → 按用户消息主导语言；非法值 → 输出 help card。不改 KB 归档路径。契约见 openspec/specs/output-language/spec.md。
