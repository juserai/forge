---
name: insight-fuse
description: "Insight Fuse v3.1 — Systematic multi-source research engine. 7-stage pipeline with skeleton.yaml data contract, 6 research-type presets, 6-dim quality rubric + 17 blocking checks (incl. primary-source binding / verbatim snippet / numeric reconciliation), and unified merged report (5-section composition)."
license: MIT
user-invokable: true
metadata:
  category: crucible
...
| `--sections` | 否 | 从 --type 预设 | report / checklist / adr / decision-tree / poc（合并为单一文件）。`--outputs` 为已弃用别名 |
...
4. 按 `--sections` 将选中段落合并为单一 markdown 文件（见 [references/output-formats.md](references/output-formats.md)）；`--outputs` 仍可用作已弃用别名
...
2. 按协议执行 KB Discovery，合并报告作为单一文件归档
...
- **段落**：新增 `templates/<output>.md` + 在 [references/output-formats.md](references/output-formats.md) 登记
