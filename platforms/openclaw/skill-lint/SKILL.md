...
控制语义检查 diagnostics（人工可读部分）输出语言；结构检查 JSON 的 rule id / 字段名不变。取值：`zh-CN`(默认) | en | ja | ko | fr | de | es | pt-BR | ru | tr | vi | auto。不带 `--lang` → zh-CN；`--lang <code>` → 强制该语言；`--lang auto` → 按用户消息语言；非法值 → 输出 help card。契约见 openspec/specs/output-language/spec.md。
