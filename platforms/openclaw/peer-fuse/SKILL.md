...
控制评审报告输出语言。取值：`zh-CN`(默认) | en | ja | ko | fr | de | es | pt-BR | ru | tr | vi | auto。不带 `--lang` → zh-CN（**覆盖**原"按源文档语言"行为）；`--lang <code>` → 强制该语言；`--lang auto` → 恢复"按源"检测（frontmatter lang → 字符比例 → fallback 中文）；非法值 → 输出 help card。契约见 openspec/specs/output-language/spec.md。
